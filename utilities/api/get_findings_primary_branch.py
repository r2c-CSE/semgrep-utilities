...
def get_all_findings(projects, headers):
    ...
    for project in projects['projects']:
        ...
        n_findings = get_findings_per_project(deployment_slug, project_name, primary_branch, headers)
        print("Getting " + str(n_findings) + " findings for " + project_name + " branch " + primary_branch)
        total_finding = total_finding + n_findings
    return total_finding
...
def get_findings_per_project(deployment_slug, project, primary_branch, headers):
    ...
    """
    project_findings = retrieve_paginated_data(f"{BASE_URL}/{deployment_slug}/findings?repos={project}&ref={primary_branch}&dedup=true", "findings", 3000, headers=headers)
    file_path = re.sub(r"[^\w\s]", "-", project) + ".json"
    with open(file_path, "w") as file:
         file.write(project_findings)
    data = json.loads(project_findings)
    number_of_findings = len(data["findings"])
...
if __name__ == "__main__":
    ...
    total_finding = get_all_findings(json.loads(projects), default_headers)
    print("Get a total of " + str(total_finding) + " findings")
