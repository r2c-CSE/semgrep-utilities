...
def process_sast_findings(df: pd.DataFrame, reportname):
    ...
    writer.close()
    logging.info(f"Generate excel report: {xlsx_filename}")
    ...
    logging.info(f"Generate html report: {html_filename}")
    ...
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            ...
            if df.empty:
                ...
                process_sast_findings(df, reportname)
        else:
            logging.info('pass the arguments like -f <findings JSON file> -h <help> or --findings <findings JSON file> and --help <help>')
            sys.exit()
